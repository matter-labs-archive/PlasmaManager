...
const ResendDelay = 1000*60*60*5 // 5 hours
const ethUtil = require("ethereumjs-util");

async function processExitChallengeFromQueue(eventName, mq, plasmaContract, storage, web3) {
    ...
    if (mess.hash === undefined) {
        ...
        const res = await processToCheck(mess, plasmaContract)
...
async function processToSend(message, plasmaContract, storage, web3) {
    try {
        ...
        const spentInInput = challenge.inputForChallenge;
        ...
        console.log("Challenging exit " + exitHash);
        console.log("It was spent in block " + spentInBlock);
        console.log("In transaction number " + spentInTx);
        console.log("In the input number " + spentInInput);
        ...
        const exitRecord = await plasmaContract.methods.exitRecords(exitHash).call();
        if (exitRecord[0] === "0x0000000000000000000000000000000000000000") {
            return null
        }
        const alreadyChallenged = exitRecord[7] == false; // isValid = false
        ...
        if (previousTransactionParameters !== undefined) {
            ...
            const abiData = plasmaContract.methods.challengeNormalExitByShowingExitBeingSpent(
                exitHash,
                spentInBlock,
                ethUtil.bufferToHex(tx.serialize()),
                ethUtil.bufferToHex(proof),
                spentInInput).encodeABI();
...
async function processToCheck(message, plasmaContract, web3) {
    try {
        ...
        const sentAt = Date.UTC(sendingTimestamp);
        const now = Date.UTC();
        ...
        const exitRecord = await plasmaContract.methods.exitRecords(exitHash).call();
        alreadyChallenged = exitRecord[7] === false;
...
module.exports = {processExitChallengeFromQueue}